          ...
          <div class="folder-scroll">
            <div class="folder-topbar">
              ...
              <div class="folder-head">
                ...
                <div v-if="folderMaterials.length" class="folder-materials">
                  <span
                    v-for="(item, idx) in folderMaterials.slice(0, 3)"
                    :key="`${item}-${idx}`"
                    class="material-chip"
                  >
                    {{ item }}
                  </span>
                  <span v-if="folderMaterials.length > 3" class="material-more">+{{ folderMaterials.length - 3 }} 个资料</span>
                </div>
              </div>
              <span class="count-pill">{{ activeFolderCards.length }} 张卡片</span>
            </div>
              ...
              <section class="card-list">
                ...
                <div v-else>
                  <div v-for="card in activeFolderCards" :key="card.id" class="card-row">
                    ...
                    <button class="icon-btn" type="button" @click.stop="openEdit(card)">
          ...
          <div class="folders-grid">
            ...
            >
              <div class="folder-card__head">
                ...
                <span class="pill">{{ getCardsForFolder(folder).length || folder.flashcardIds.length }} 张</span>
              </div>
            </div>
          </div>
...
import { ArrowLeftIcon, Edit3Icon, ShuffleIcon, SparklesIcon } from 'lucide-vue-next'
...
const activeFolderCards = computed(() => {
  ...
  return cardsByFolderMap.value.get(selectedFolder.value.id) ?? []
})
...
:deep(.rounded-modal .ant-modal-content) {
  border-radius: 28px;
  overflow: hidden;
}

:deep(.rounded-modal .ant-modal-header) {
  border-radius: 28px 28px 0 0;
  padding: 20px 24px;
}

:deep(.rounded-modal .ant-modal-body) {
  padding: 20px 24px;
}

:deep(.rounded-modal .ant-modal-footer) {
  border-radius: 0 0 28px 28px;
  padding: 16px 24px;
}

:deep(.rounded-modal .ant-modal-title) {
  font-size: 18px;
  font-weight: 700;
}
